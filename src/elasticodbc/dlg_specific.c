...
static char *makeKeepaliveConnectString(char *target, int buflen,
                                        const ConnInfo *ci, BOOL abbrev) {
    char *buf = target;
    *buf = '\0';
    ...
    if (ci->keepalive_idle >= 0) {
        if (abbrev)
            snprintf(buf, buflen, ABBR_KEEPALIVETIME "=%u;",
                     ci->keepalive_idle);
        else
    ...
    if (ci->keepalive_interval >= 0) {
        if (abbrev)
            snprintfcat(buf, buflen, ABBR_KEEPALIVEINTERVAL "=%u;",
                        ci->keepalive_interval);
        else
            snprintfcat(buf, buflen, INI_KEEPALIVEINTERVAL "=%u;",
                        ci->keepalive_interval);
    }
    return target;
}
...
char *makeXaOptConnectString(char *target, int buflen, const ConnInfo *ci,
                             BOOL abbrev) {
    char *buf = target;
    *buf = '\0';
...
void makeConnectString(char *connect_string, const ConnInfo *ci, UWORD len) {
    ...
    olen = snprintf(
        ...
        "ResponseTimeout=%s;" INI_SSL_USE "=%d;" INI_SSL_VERIFY_SERVER
        "=%d;" INI_SSL_ALLOW_SELF_SIGNED "=%d;" INI_SSL_CERTIFICATE
        "=%s;" INI_SSL_KEY " =%s;" INI_AUTH_MODE "=%s;" INI_REGION "=%s;",
        got_dsn ? "DSN" : "DRIVER", got_dsn ? ci->dsn : ci->drivername,
        ci->database, ci->server, ci->port, ci->username, encoded_item,
        ci->response_timeout, (int)ci->use_ssl, (int)ci->verify_server,
        (int)ci->trust_self_signed, ci->certificate, ci->key, ci->authtype, ci->region);
    if (olen < 0 || olen >= nlen) {
        connect_string[0] = '\0';
        return;
    }
    ...
    if (!abbrev) {
        ...
        olen = snprintf(
            &connect_string[hlen], nlen,
            ";" INI_READONLY "=%s;" INI_PROTOCOL "=%s;" INI_FAKEOIDINDEX
            "=%s;" INI_SHOWOIDCOLUMN "=%s;" INI_ROWVERSIONING
            "=%s;" INI_SHOWSYSTEMTABLES
            "=%s;"
            "%s" /* INI_CONNSETTINGS */
            INI_FETCH "=%d;" INI_UNKNOWNSIZES "=%d;" INI_MAXVARCHARSIZE
            "=%d;" INI_MAXLONGVARCHARSIZE "=%d;" INI_DEBUG "=%d;" INI_COMMLOG
            "=%d;" INI_USEDECLAREFETCH "=%d;" INI_TEXTASLONGVARCHAR
            "=%d;" INI_UNKNOWNSASLONGVARCHAR "=%d;" INI_BOOLSASCHAR
            "=%d;" INI_PARSE "=%d;" INI_EXTRASYSTABLEPREFIXES
            "=%s;" INI_LFCONVERSION "=%d;" INI_UPDATABLECURSORS
            "=%d;" INI_TRUEISMINUS1 "=%d;" INI_INT8AS
            "=%d;" INI_BYTEAASLONGVARBINARY "=%d;" INI_USESERVERSIDEPREPARE
            "=%d;" INI_LOWERCASEIDENTIFIER
            "=%d;"
            "%s" /* INI_ESOPT */
...
#endif                      /* _HANDLE_ENLIST_IN_DTC_ */
            ,
            ci->onlyread, protocol_and, ci->fake_oid_index, ci->show_oid_column,
            ci->row_versioning, ci->show_system_tables,
            makeBracketConnectString(ci->conn_settings_in_str, &connsetStr,
                                     ci->conn_settings, INI_CONNSETTINGS),
            ci->drivers.fetch_max, ci->drivers.unknown_sizes,
            ci->drivers.max_varchar_size, ci->drivers.max_longvarchar_size,
            ci->drivers.debug, ci->drivers.commlog,
            ci->drivers.use_declarefetch, ci->drivers.text_as_longvarchar,
            ci->drivers.unknowns_as_longvarchar, ci->drivers.bools_as_char,
            ci->drivers.parse, ci->drivers.extra_systable_prefixes,
            ci->lf_conversion, ci->allow_keyset, ci->true_is_minus1,
            ci->int8_as, ci->bytea_as_longvarbinary,
            ci->use_server_side_prepare, ci->lower_case_identifier,
            makeBracketConnectString(ci->esopt_in_str, &esoptStr, ci->esopt,
                                     INI_ESOPT),
            makeKeepaliveConnectString(keepaliveStr, sizeof(keepaliveStr), ci,
                                       FALSE),
            ci->numeric_as
#ifdef _HANDLE_ENLIST_IN_DTC_
            ,
    ...
    if (abbrev || olen >= nlen || olen < 0) {
        ...
        olen = snprintf(
            ...
            ";"
            "%s" /* ABBR_CONNSETTINGS */
            ABBR_FETCH "=%d;" ABBR_MAXVARCHARSIZE "=%d;" ABBR_MAXLONGVARCHARSIZE
            "=%d;" INI_INT8AS "=%d;" ABBR_EXTRASYSTABLEPREFIXES
            "=%s;"
            "%s" /* ABBR_ESOPT */
            ...
            makeBracketConnectString(ci->conn_settings_in_str, &connsetStr,
                                     ci->conn_settings, ABBR_CONNSETTINGS),
            ci->drivers.fetch_max, ci->drivers.max_varchar_size,
            ci->drivers.max_longvarchar_size, ci->int8_as,
...
static void unfoldCXAttribute(ConnInfo *ci, const char *value) {
    int count;
    UInt4 flag;
    ...
    ci->drivers.text_as_longvarchar =
        (char)((flag & BIT_TEXTASLONGVARCHAR) != 0);
    ci->drivers.unknowns_as_longvarchar =
        (char)((flag & BIT_UNKNOWNSASLONGVARCHAR) != 0);
    ci->drivers.bools_as_char = (char)((flag & BIT_BOOLSASCHAR) != 0);
    ITOA_FIXED(ci->row_versioning, (char)((flag & BIT_ROWVERSIONING) != 0));
    ITOA_FIXED(ci->show_system_tables,
               (char)((flag & BIT_SHOWSYSTEMTABLES) != 0));
    ...
    ci->use_server_side_prepare =
        (char)((flag & BIT_USESERVERSIDEPREPARE) != 0);
...
BOOL copyConnAttributes(ConnInfo *ci, const char *attribute,
    ...
    if (stricmp(attribute, "DSN") == 0)
        ...
        STRCPY_FIXED(ci->desc, value);
             ...
             || stricmp(attribute, ABBR_DATABASE) == 0)
    ...
    else if (stricmp(attribute, INI_SSL_CERTIFICATE) == 0)
        STRCPY_FIXED(ci->certificate, value);
    else if (stricmp(attribute, INI_SSL_KEY) == 0)
        STRCPY_FIXED(ci->key, value);
        ...
        if (ptr) {
            ...
            ci->rollback_on_error = (char)atoi(ptr + 1);
            MYLOG(0, "key='%s' value='%s' rollback_on_error=%d\n", attribute,
                  value, ci->rollback_on_error);
            printed = TRUE;
        }
        ...
        STRCPY_FIXED(ci->show_oid_column, value);
             ...
             || stricmp(attribute, ABBR_FAKEOIDINDEX) == 0)
        STRCPY_FIXED(ci->fake_oid_index, value);
    else if (stricmp(attribute, INI_ROWVERSIONING) == 0
             || stricmp(attribute, ABBR_ROWVERSIONING) == 0)
        STRCPY_FIXED(ci->row_versioning, value);
    else if (stricmp(attribute, INI_SHOWSYSTEMTABLES) == 0
             || stricmp(attribute, ABBR_SHOWSYSTEMTABLES) == 0)
        STRCPY_FIXED(ci->show_system_tables, value);
    else if (stricmp(attribute, INI_CONNSETTINGS) == 0
             || stricmp(attribute, ABBR_CONNSETTINGS) == 0) {
        /* We can use the conn_settings directly when they are enclosed with
         * braces */
        ci->conn_settings_in_str = TRUE;
        ci->conn_settings = decode_or_remove_braces(value);
    } else if ((stricmp(attribute, INI_ESOPT) == 0)
               || (stricmp(attribute, ABBR_ESOPT) == 0)) {
        ci->esopt_in_str = TRUE;
        ci->esopt = decode_or_remove_braces(value);
    } else if (stricmp(attribute, INI_UPDATABLECURSORS) == 0
               || stricmp(attribute, ABBR_UPDATABLECURSORS) == 0)
        ci->allow_keyset = (char)atoi(value);
    else if (stricmp(attribute, INI_LFCONVERSION) == 0
             || stricmp(attribute, ABBR_LFCONVERSION) == 0)
        ci->lf_conversion = (char)atoi(value);
    else if (stricmp(attribute, INI_TRUEISMINUS1) == 0
             || stricmp(attribute, ABBR_TRUEISMINUS1) == 0)
        ci->true_is_minus1 = (char)atoi(value);
    else if (stricmp(attribute, INI_INT8AS) == 0)
        ci->int8_as = (char)atoi(value);
    else if (stricmp(attribute, INI_NUMERIC_AS) == 0)
        ci->numeric_as = (char)atoi(value);
    else if (stricmp(attribute, INI_BYTEAASLONGVARBINARY) == 0
             || stricmp(attribute, ABBR_BYTEAASLONGVARBINARY) == 0)
        ci->bytea_as_longvarbinary = (char)atoi(value);
    else if (stricmp(attribute, INI_USESERVERSIDEPREPARE) == 0
             || stricmp(attribute, ABBR_USESERVERSIDEPREPARE) == 0)
        ci->use_server_side_prepare = (char)atoi(value);
    else if (stricmp(attribute, INI_LOWERCASEIDENTIFIER) == 0
             || stricmp(attribute, ABBR_LOWERCASEIDENTIFIER) == 0)
        ci->lower_case_identifier = (char)atoi(value);
    else if (stricmp(attribute, INI_KEEPALIVETIME) == 0
             || stricmp(attribute, ABBR_KEEPALIVETIME) == 0)
        ci->keepalive_idle = atoi(value);
    else if (stricmp(attribute, INI_KEEPALIVEINTERVAL) == 0
             || stricmp(attribute, ABBR_KEEPALIVEINTERVAL) == 0)
        ...
        MYLOG(0,
              "key='%s' value='%s'(force_abbrev=%d bde=%d cvt_null_date=%x)\n",
              attribute, value, ci->force_abbrev_connstr, ci->bde_environment,
              ci->cvt_null_date_string);
        printed = TRUE;
    }

    else if (stricmp(attribute, INI_FETCH) == 0
             || stricmp(attribute, ABBR_FETCH) == 0)
        ci->drivers.fetch_max = atoi(value);
    else if (stricmp(attribute, INI_DEBUG) == 0
             || stricmp(attribute, ABBR_DEBUG) == 0)
        ci->drivers.debug = (char)atoi(value);
    else if (stricmp(attribute, INI_COMMLOG) == 0
             || stricmp(attribute, ABBR_COMMLOG) == 0)
        ci->drivers.commlog = (char)atoi(value);
    /*
     ...
     */
             ...
             || stricmp(attribute, ABBR_UNKNOWNSIZES) == 0)
        ci->drivers.unknown_sizes = atoi(value);
    else if (stricmp(attribute, INI_LIE) == 0)
        ci->drivers.lie = (char)atoi(value);
    else if (stricmp(attribute, INI_PARSE) == 0
             || stricmp(attribute, ABBR_PARSE) == 0)
        ci->drivers.parse = (char)atoi(value);
    else if (stricmp(attribute, INI_USEDECLAREFETCH) == 0
             || stricmp(attribute, ABBR_USEDECLAREFETCH) == 0)
        ci->drivers.use_declarefetch = (char)atoi(value);
    else if (stricmp(attribute, INI_MAXVARCHARSIZE) == 0
             || stricmp(attribute, ABBR_MAXVARCHARSIZE) == 0)
        ci->drivers.max_varchar_size = atoi(value);
    else if (stricmp(attribute, INI_MAXLONGVARCHARSIZE) == 0
             || stricmp(attribute, ABBR_MAXLONGVARCHARSIZE) == 0)
        ci->drivers.max_longvarchar_size = atoi(value);
    else if (stricmp(attribute, INI_TEXTASLONGVARCHAR) == 0
             || stricmp(attribute, ABBR_TEXTASLONGVARCHAR) == 0)
        ci->drivers.text_as_longvarchar = (char)atoi(value);
    else if (stricmp(attribute, INI_UNKNOWNSASLONGVARCHAR) == 0
             || stricmp(attribute, ABBR_UNKNOWNSASLONGVARCHAR) == 0)
        ci->drivers.unknowns_as_longvarchar = (char)atoi(value);
    else if (stricmp(attribute, INI_BOOLSASCHAR) == 0
             || stricmp(attribute, ABBR_BOOLSASCHAR) == 0)
        ci->drivers.bools_as_char = (char)atoi(value);
    else if (stricmp(attribute, INI_EXTRASYSTABLEPREFIXES) == 0
             || stricmp(attribute, ABBR_EXTRASYSTABLEPREFIXES) == 0)
...
static void getCiDefaults(ConnInfo *ci) {
    ...
    ci->trust_self_signed = DEFAULT_TRUST_SELF_SIGNED;
    strcpy(ci->authtype, DEFAULT_AUTH_MODE);
    strcpy(ci->region, DEFAULT_REGION);
    strcpy(ci->certificate, DEFAULT_CERTIFICATE);
    strcpy(ci->key, DEFAULT_KEY);
    ...
    ci->disable_keepalive = 0;
    {
        ...
        ci->wcs_debug = 0;
        if (NULL != (p = getenv("ELASTICSEARCHODBC_WCS_DEBUG")))
            if (strcmp(p, "1") == 0)
                ci->wcs_debug = 1;
    }
#ifdef _HANDLE_ENLIST_IN_DTC_
...
static void Global_defset(GLOBAL_VALUES *comval) {
    ...
    STRCPY_FIXED(comval->extra_systable_prefixes,
                 DEFAULT_EXTRASYSTABLEPREFIXES);
    STRCPY_FIXED(comval->protocol, DEFAULT_PROTOCOL);
}

static void get_Ci_Drivers(const char *section, const char *filename,
...
void getDSNinfo(ConnInfo *ci, const char *configDrvrname) {
    ...
    SQLGetPrivateProfileString(DSN, INI_KDESC, NULL_STRING, ci->desc,
                               sizeof(ci->desc), ODBC_INI);
    ...
    if (SQLGetPrivateProfileString(DSN, INI_DATABASE, NULL_STRING, temp,
                                   sizeof(temp), ODBC_INI)
        > 0)
    ...
    if (SQLGetPrivateProfileString(DSN, INI_SSL_VERIFY_SERVER, NULL_STRING,
    ...
    if (SQLGetPrivateProfileString(DSN, INI_DEBUG, NULL_STRING, temp,
                                   sizeof(temp), ODBC_INI)
        > 0)
        ci->drivers.debug = (char)atoi(temp);
    if (SQLGetPrivateProfileString(DSN, INI_COMMLOG, NULL_STRING, temp,
                                   sizeof(temp), ODBC_INI)
        > 0)
        ci->drivers.commlog = (char)atoi(temp);

    if (SQLGetPrivateProfileString(DSN, INI_READONLY, NULL_STRING, temp,
                                   sizeof(temp), ODBC_INI)
        > 0)
        STRCPY_FIXED(ci->onlyread, temp);

    if (SQLGetPrivateProfileString(DSN, INI_SHOWOIDCOLUMN, NULL_STRING, temp,
                                   sizeof(temp), ODBC_INI)
        > 0)
        STRCPY_FIXED(ci->show_oid_column, temp);

    if (SQLGetPrivateProfileString(DSN, INI_FAKEOIDINDEX, NULL_STRING, temp,
                                   sizeof(temp), ODBC_INI)
        > 0)
        STRCPY_FIXED(ci->fake_oid_index, temp);

    if (SQLGetPrivateProfileString(DSN, INI_ROWVERSIONING, NULL_STRING, temp,
                                   sizeof(temp), ODBC_INI)
        > 0)
        STRCPY_FIXED(ci->row_versioning, temp);

    if (SQLGetPrivateProfileString(DSN, INI_SHOWSYSTEMTABLES, NULL_STRING, temp,
                                   sizeof(temp), ODBC_INI)
        > 0)
        STRCPY_FIXED(ci->show_system_tables, temp);

    SQLGetPrivateProfileString(DSN, INI_PROTOCOL, ENTRY_TEST, temp,
                               sizeof(temp), ODBC_INI);
    ...
    SQLGetPrivateProfileString(DSN, INI_CONNSETTINGS, ENTRY_TEST, temp,
                               sizeof(temp), ODBC_INI);
    if (strcmp(temp, ENTRY_TEST)) /* entry exists */
    {
        const UCHAR *ptr;
    ...
    SQLGetPrivateProfileString(DSN, INI_ESOPT, ENTRY_TEST, temp, sizeof(temp),
                               ODBC_INI);
    if (strcmp(temp, ENTRY_TEST)) /* entry exists */
        STRX_TO_NAME(ci->esopt, temp);

    if (SQLGetPrivateProfileString(DSN, INI_TRANSLATIONDLL, NULL_STRING, temp,
                                   sizeof(temp), ODBC_INI)
        > 0)
        STRCPY_FIXED(ci->translation_dll, temp);

    if (SQLGetPrivateProfileString(DSN, INI_TRANSLATIONOPTION, NULL_STRING,
                                   temp, sizeof(temp), ODBC_INI)
        > 0)
        STRCPY_FIXED(ci->translation_option, temp);

    if (SQLGetPrivateProfileString(DSN, INI_UPDATABLECURSORS, NULL_STRING, temp,
                                   sizeof(temp), ODBC_INI)
        > 0)
        ci->allow_keyset = (char)atoi(temp);

    if (SQLGetPrivateProfileString(DSN, INI_LFCONVERSION, NULL_STRING, temp,
                                   sizeof(temp), ODBC_INI)
        > 0)
        ci->lf_conversion = (char)atoi(temp);

    if (SQLGetPrivateProfileString(DSN, INI_TRUEISMINUS1, NULL_STRING, temp,
                                   sizeof(temp), ODBC_INI)
        > 0)
        ci->true_is_minus1 = (char)atoi(temp);

    if (SQLGetPrivateProfileString(DSN, INI_INT8AS, NULL_STRING, temp,
                                   sizeof(temp), ODBC_INI)
        > 0)
        ci->int8_as = (char)atoi(temp);

    if (SQLGetPrivateProfileString(DSN, INI_NUMERIC_AS, NULL_STRING, temp,
                                   sizeof(temp), ODBC_INI)
        > 0)
        ci->numeric_as = (char)atoi(temp);

    if (SQLGetPrivateProfileString(DSN, INI_BYTEAASLONGVARBINARY, NULL_STRING,
                                   temp, sizeof(temp), ODBC_INI)
        > 0)
        ci->bytea_as_longvarbinary = (char)atoi(temp);

    if (SQLGetPrivateProfileString(DSN, INI_USESERVERSIDEPREPARE, NULL_STRING,
                                   temp, sizeof(temp), ODBC_INI)
        > 0)
        ci->use_server_side_prepare = (char)atoi(temp);

    if (SQLGetPrivateProfileString(DSN, INI_LOWERCASEIDENTIFIER, NULL_STRING,
                                   temp, sizeof(temp), ODBC_INI)
        > 0)
        ci->lower_case_identifier = (char)atoi(temp);

    if (SQLGetPrivateProfileString(DSN, INI_KEEPALIVETIME, NULL_STRING, temp,
                                   sizeof(temp), ODBC_INI)
        > 0)
        if (0 == (ci->keepalive_idle = atoi(temp)))
            ci->keepalive_idle = -1;
    if (SQLGetPrivateProfileString(DSN, INI_KEEPALIVEINTERVAL, NULL_STRING,
                                   temp, sizeof(temp), ODBC_INI)
        > 0)
...
#ifdef _HANDLE_ENLIST_IN_DTC_
    if (SQLGetPrivateProfileString(DSN, INI_XAOPT, NULL_STRING, temp,
                                   sizeof(temp), ODBC_INI)
        > 0)
        ci->xa_opt = atoi(temp);
#endif /* _HANDLE_ENLIST_IN_DTC_ */
    ...
    if (SQLGetPrivateProfileString(DSN, INI_EXTRAOPTIONS, NULL_STRING, temp,
                                   sizeof(temp), ODBC_INI)
        > 0) {
        UInt4 val = 0;
    ...
    MYLOG(DETAIL_LOG_LEVEL,
          "DSN info: "
          "DSN='%s',server='%s',port='%s',dbase='%s',user='%s',passwd='%s'\n",
          DSN, ci->server, ci->port, ci->database, ci->username,
          NAME_IS_VALID(ci->password) ? "xxxxx" : "");
    MYLOG(DETAIL_LOG_LEVEL,
          "          "
          "onlyread='%s',showoid='%s',fakeoidindex='%s',showsystable='%s'\n",
          ci->onlyread, ci->show_oid_column, ci->fake_oid_index,
...
int write_Ci_Drivers(const char *fileName, const char *sectionName,
    ...
    if (!SQLWritePrivateProfileString(sectionName, INI_UNIQUEINDEX, tmp,
                                      fileName))
        errc--;

    ITOA_FIXED(tmp, comval->use_declarefetch);
    if (!SQLWritePrivateProfileString(sectionName, INI_USEDECLAREFETCH, tmp,
                                      fileName))
        errc--;

    ITOA_FIXED(tmp, comval->unknown_sizes);
    if (!SQLWritePrivateProfileString(sectionName, INI_UNKNOWNSIZES, tmp,
                                      fileName))
        errc--;

    ITOA_FIXED(tmp, comval->text_as_longvarchar);
    if (!SQLWritePrivateProfileString(sectionName, INI_TEXTASLONGVARCHAR, tmp,
                                      fileName))
        errc--;

    ITOA_FIXED(tmp, comval->unknowns_as_longvarchar);
    if (!SQLWritePrivateProfileString(sectionName, INI_UNKNOWNSASLONGVARCHAR,
                                      tmp, fileName))
        errc--;

    ITOA_FIXED(tmp, comval->bools_as_char);
    if (!SQLWritePrivateProfileString(sectionName, INI_BOOLSASCHAR, tmp,
                                      fileName))
        errc--;
    ...
    if (!SQLWritePrivateProfileString(sectionName, INI_MAXVARCHARSIZE, tmp,
                                      fileName))
        errc--;

    ITOA_FIXED(tmp, comval->max_longvarchar_size);
    if (!SQLWritePrivateProfileString(sectionName, INI_MAXLONGVARCHARSIZE, tmp,
                                      fileName))
        errc--;

    if (!SQLWritePrivateProfileString(sectionName, INI_EXTRASYSTABLEPREFIXES,
...
void writeDSNinfo(const ConnInfo *ci) {
    ...
    SQLWritePrivateProfileString(DSN, INI_SSL_CERTIFICATE, ci->certificate,
                                 ODBC_INI);
    SQLWritePrivateProfileString(DSN, INI_SSL_KEY, ci->key, ODBC_INI);
    ITOA_FIXED(temp, ci->trust_self_signed);
    SQLWritePrivateProfileString(DSN, INI_SSL_ALLOW_SELF_SIGNED, temp,
                                 ODBC_INI);
    ITOA_FIXED(temp, ci->use_ssl);
    SQLWritePrivateProfileString(DSN, INI_SSL_USE, temp, ODBC_INI);
    ITOA_FIXED(temp, ci->verify_server);
    SQLWritePrivateProfileString(DSN, INI_SSL_VERIFY_SERVER, temp, ODBC_INI);
    ...
    SQLWritePrivateProfileString(DSN, INI_SHOWOIDCOLUMN, ci->show_oid_column,
                                 ODBC_INI);

    SQLWritePrivateProfileString(DSN, INI_FAKEOIDINDEX, ci->fake_oid_index,
                                 ODBC_INI);

    SQLWritePrivateProfileString(DSN, INI_ROWVERSIONING, ci->row_versioning,
                                 ODBC_INI);

    SQLWritePrivateProfileString(DSN, INI_SHOWSYSTEMTABLES,
                                 ci->show_system_tables, ODBC_INI);

    if (ci->rollback_on_error >= 0)
        SPRINTF_FIXED(temp, "7.4-%d", ci->rollback_on_error);
    else
        STRCPY_FIXED(temp, NULL_STRING);
    SQLWritePrivateProfileString(DSN, INI_PROTOCOL, temp, ODBC_INI);

    SQLWritePrivateProfileString(DSN, INI_CONNSETTINGS,
                                 SAFE_NAME(ci->conn_settings), ODBC_INI);

    SQLWritePrivateProfileString(DSN, INI_ESOPT, SAFE_NAME(ci->esopt),
                                 ODBC_INI);

    ITOA_FIXED(temp, ci->allow_keyset);
    SQLWritePrivateProfileString(DSN, INI_UPDATABLECURSORS, temp, ODBC_INI);
    ITOA_FIXED(temp, ci->lf_conversion);
    SQLWritePrivateProfileString(DSN, INI_LFCONVERSION, temp, ODBC_INI);
    ITOA_FIXED(temp, ci->true_is_minus1);
    SQLWritePrivateProfileString(DSN, INI_TRUEISMINUS1, temp, ODBC_INI);
    ITOA_FIXED(temp, ci->int8_as);
    SQLWritePrivateProfileString(DSN, INI_INT8AS, temp, ODBC_INI);
    ITOA_FIXED(temp, ci->numeric_as);
    SQLWritePrivateProfileString(DSN, INI_NUMERIC_AS, temp, ODBC_INI);
    SPRINTF_FIXED(temp, "%x", getExtraOptions(ci));
    SQLWritePrivateProfileString(DSN, INI_EXTRAOPTIONS, temp, ODBC_INI);
    ITOA_FIXED(temp, ci->bytea_as_longvarbinary);
    SQLWritePrivateProfileString(DSN, INI_BYTEAASLONGVARBINARY, temp, ODBC_INI);
    ITOA_FIXED(temp, ci->use_server_side_prepare);
    SQLWritePrivateProfileString(DSN, INI_USESERVERSIDEPREPARE, temp, ODBC_INI);
    ITOA_FIXED(temp, ci->lower_case_identifier);
    SQLWritePrivateProfileString(DSN, INI_LOWERCASEIDENTIFIER, temp, ODBC_INI);
    ITOA_FIXED(temp, ci->keepalive_idle);
    SQLWritePrivateProfileString(DSN, INI_KEEPALIVETIME, temp, ODBC_INI);
    ITOA_FIXED(temp, ci->keepalive_interval);
    SQLWritePrivateProfileString(DSN, INI_KEEPALIVEINTERVAL, temp, ODBC_INI);
#ifdef _HANDLE_ENLIST_IN_DTC_
...
static void get_Ci_Drivers(const char *section, const char *filename,
    ...
    if (SQLGetPrivateProfileString(section, INI_FETCH, NULL_STRING, temp,
    ...
    if (SQLGetPrivateProfileString(section, INI_UNIQUEINDEX, NULL_STRING, temp,
                                   sizeof(temp), filename)
        > 0)
        comval->unique_index = (char)atoi(temp);

    /* Unknown Sizes is stored in the driver section only */
    if (SQLGetPrivateProfileString(section, INI_UNKNOWNSIZES, NULL_STRING, temp,
                                   sizeof(temp), filename)
        > 0)
        comval->unknown_sizes = atoi(temp);

    /* Lie about supported functions? */
    if (SQLGetPrivateProfileString(section, INI_LIE, NULL_STRING, temp,
                                   sizeof(temp), filename)
        > 0)
        comval->lie = (char)atoi(temp);

    /* Parse statements */
    if (SQLGetPrivateProfileString(section, INI_PARSE, NULL_STRING, temp,
                                   sizeof(temp), filename)
        > 0)
        comval->parse = (char)atoi(temp);

    /* UseDeclareFetch is stored in the driver section only */
    if (SQLGetPrivateProfileString(section, INI_USEDECLAREFETCH, NULL_STRING,
                                   temp, sizeof(temp), filename)
        > 0)
        comval->use_declarefetch = (char)atoi(temp);
    ...
    if (SQLGetPrivateProfileString(section, INI_TEXTASLONGVARCHAR, NULL_STRING,
                                   temp, sizeof(temp), filename)
        > 0)
        comval->text_as_longvarchar = (char)atoi(temp);

    /* Unknowns As LongVarchar	*/
    if (SQLGetPrivateProfileString(section, INI_UNKNOWNSASLONGVARCHAR,
                                   NULL_STRING, temp, sizeof(temp), filename)
        > 0)
        comval->unknowns_as_longvarchar = (char)atoi(temp);

    /* Bools As Char */
    if (SQLGetPrivateProfileString(section, INI_BOOLSASCHAR, NULL_STRING, temp,
                                   sizeof(temp), filename)
        > 0)
        comval->bools_as_char = (char)atoi(temp);
    ...
    if (inst_position) {
         ...
         */
        SQLGetPrivateProfileString(section, INI_PROTOCOL, ENTRY_TEST, temp,
                                   sizeof(temp), filename);
        if (strcmp(temp, ENTRY_TEST))
...
signed char ci_updatable_cursors_set(ConnInfo *ci) {
    ...
    if (ci->allow_keyset) {
        if (ci->drivers.lie || !ci->drivers.use_declarefetch)
            ...
            ci->updatable_cursors |=
                (ALLOW_STATIC_CURSORS | ALLOW_BULK_OPERATIONS
...
void CC_conninfo_init(ConnInfo *conninfo, UInt4 option) {
    ...
    strncpy(conninfo->response_timeout, DEFAULT_RESPONSE_TIMEOUT_STR,
            SMALL_REGISTRY_LEN);
    strncpy(conninfo->certificate, DEFAULT_CERTIFICATE, MEDIUM_REGISTRY_LEN);
    strncpy(conninfo->key, DEFAULT_KEY, MEDIUM_REGISTRY_LEN);
    ...
    conninfo->trust_self_signed = DEFAULT_TRUST_SELF_SIGNED;
    conninfo->allow_keyset = 0;
...
void CC_copy_conninfo(ConnInfo *ci, const ConnInfo *sci) {
    ...
    CORR_STRCPY(region);
    CORR_STRCPY(certificate);
    CORR_STRCPY(key);
    NAME_TO_NAME(ci->password, sci->password);
    CORR_VALCPY(trust_self_signed);
    CORR_VALCPY(use_ssl);
    ...
    CORR_VALCPY(xa_opt);
#endif
    copy_globals(&(ci->drivers),
                 &(sci->drivers)); /* moved from driver's option */
}
#undef CORR_STRCPY
#undef CORR_VALCPY
