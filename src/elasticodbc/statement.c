...
RETCODE SQL_API ESAPI_FreeStmt(HSTMT hstmt, SQLUSMALLINT fOption) {
    ...
    MYLOG(ES_Debug, "entering...hstmt=%p, fOption=%hi\n", hstmt, fOption);
...
char SC_Destructor(StatementClass *self) {
    ...
    MYLOG(ES_Debug, "entering self=%p, self->result=%p, self->hdbc=%p\n", self,
          res, self->hdbc);
    SC_clear_error(self);
...
void SC_error_copy(StatementClass *self, const StatementClass *from,
    ...
    MYLOG(ES_Debug, "entering %p->%p check=%i\n", from, self, check);
    if (!from)
        return; /* for safety */
    if (self == from)
        return; /* for safety */
...
SC_fetch(StatementClass *self) {
    ...
    MYLOG(ES_Debug, "entering statement=%p res=%p ommitted=0\n", self, res);
    self->last_fetch_count = self->last_fetch_count_include_ommitted = 0;
    if (!res)
        return SQL_ERROR;
...
RETCODE dequeueNeedDataCallback(RETCODE retcode, StatementClass *stmt) {
    ...
    MYLOG(ES_Debug, "entering ret=%d count=%d\n", retcode, stmt->num_callbacks);
...
int SC_Create_bookmark(StatementClass *self, BindInfoClass *bookmark,
    ...
    MYLOG(ES_Debug, "entering type=%d buflen=" FORMAT_LEN " buf=%p\n",
    ...
    MYLOG(ES_Debug, "leaving cvtlen=" FORMAT_SIZE_T " ix(bl,of)=%d(%d,%d)\n",
