...
void TI_Destructor(TABLE_INFO **ti, int count) {
    ...
    MYLOG(ES_Debug, "entering count=%d\n", count);
    if (ti) {
        for (i = 0; i < count; i++) {
            if (ti[i]) {
...
static void ARDFields_free(ARDFields *self) {
    MYLOG(ES_All, "entering %p bookmark=%p\n", self, self->bookmark);
    if (self->bookmark) {
        free(self->bookmark);
        self->bookmark = NULL;
    }
...
char CC_add_descriptor(ConnectionClass *self, DescriptorClass *desc) {
    ...
    MYLOG(ES_Debug, "entering self=%p, desc=%p\n", self, desc);
...
RETCODE SQL_API ESAPI_DescError(SQLHDESC hdesc, SQLSMALLINT RecNumber,
    ...
    MYLOG(ES_Debug, "entering RecN=%hd\n", RecNumber);
